...
| `masonmem/ai-sync` (was `masonmem/copilot`) | Shared AI brain — skills, agents, MCP wrappers, instructions; consumed by both Copilot CLI (`~/.copilot/`) and Claude Code (`~/.claude/`) via symlinks into `~/code/ai-sync/` | Service deploys |
...
| MCP server (user-global) | `~/code/ai-sync/mcp/servers.toml` (single source of truth) + optional `~/code/ai-sync/bin/<name>-mcp-wrapper.sh` for secrets, then `ai-sync apply` (never hand-edit the generated `mcp.json` or settings.json `mcpServers`) | Copilot `/mcp reload`; Claude Code session restart |
| MCP server (project) | `<repo>/.github/mcp.json` (Copilot) or `<repo>/.mcp.json` (Claude Code) | Per-project |
| AI skill (global) | `~/code/ai-sync/skills/<name>/SKILL.md` (consumed by both Copilot and Claude Code) | Copilot `/skills reload`; Claude Code session restart |
...
| BYOK key for local AI CLI tools (Copilot/Claude Code/etc.) | `~/code/ai-sync/secrets/<provider>.env` (per `dotfiles-helper`) — these are *client-side*, not the gateway | — |
...
8. **LiteLLM config changes** flow through GitOps automatically: the `GitOps Auto-Deploy` Komodo procedure (every 5 min) first runs the `komodo-homelab` ResourceSync, then its action diffs each stack's runtime inputs in Core's git cache. Runtime inputs are the compose `file_paths` plus explicit additions in `EXTRA_WATCH_PATHS`; LiteLLM adds `config.yaml`, so a config-only commit triggers `DeployStack` and a follow-up `RestartStack` to force re-read of the bind-mounted config (litellm's in-process `model_group` cache otherwise serves stale data). The detection and post-deploy hook live in `komodo/resources/actions.toml` → `auto-deploy-pending-stacks`. Manual override (skip the wait): `python3 scripts/komodo.py deploy litellm && ssh solaris '/opt/homebrew/bin/docker restart litellm'`.
