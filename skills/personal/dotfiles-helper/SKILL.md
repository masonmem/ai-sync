...
description: Encodes the conventions of @masonmem's dotfiles + ~/code/ai-sync setup. Use when the user asks to install a CLI tool, add an alias, configure a shell env var, add a new MCP server, add a custom skill or agent, or otherwise modify their machine environment so it stays reproducible across machines.
---
...
- **`~/code/ai-sync`** (`masonmem/ai-sync`) — shared AI brain consumed by **Claude Code** (via `~/.claude/`), **Copilot CLI** (via `~/.copilot/`), and **Codex** (via `~/.codex/AGENTS.md`). Holds global instructions (`agents/general.md`), skills (`skills/general/`, `skills/personal/`), MCP wrapper scripts, per-tool settings, and secrets. Plain clone (no stow); `~/code/ai-sync` is a compatibility symlink to it.
...
```text
~/code/ai-sync/skills/<lowercase-hyphenated-name>/
