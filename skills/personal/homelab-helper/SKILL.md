...
8. **LiteLLM config changes** flow through GitOps automatically: the `GitOps Auto-Deploy` Komodo procedure (every 5 min) first runs the `komodo-homelab` ResourceSync, then its action diffs each stack's full `run_directory` in Core's git cache (not just the compose `file_paths`). A `solaris/litellm/config.yaml`-only commit therefore triggers `DeployStack` and — for litellm specifically — a follow-up `RestartStack` to force re-read of the bind-mounted config (litellm's in-process `model_group` cache otherwise serves stale data). The detection and post-deploy hook live in `komodo/resources/actions.toml` → `auto-deploy-pending-stacks`. Manual override (skip the wait): `python3 scripts/komodo.py deploy litellm && ssh solaris '/opt/homebrew/bin/docker restart litellm'`.
