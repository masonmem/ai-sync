...
| MCP server (user-global) | `~/.ai-config/mcp.json` (Copilot) + `~/.ai-config/claude/settings.json` mcpServers (Claude Code), plus wrapper in `~/.ai-config/bin/` for secrets | Copilot `/mcp reload`; Claude Code session restart |
